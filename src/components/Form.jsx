...
function Form({ register, login }) {
  ...
  const loginForm = async (e) => {
    ...
    form = Object.fromEntries(form)
    try {
      await axios.post(`${import.meta.env.VITE_API_URL}/user/auth`, form, { withCredentials: true,headers:'Access-Control-Allow-Origin' })
      toast.dismiss(loading)
      const { data } = await axios.post(`${import.meta.env.VITE_API_URL}/user/singleUser`, {}, { withCredentials: true,headers:'Access-Control-Allow-Origin' })
